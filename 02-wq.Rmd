...
```{r wq_options, include=FALSE, warning=FALSE, message=FALSE, comment=FALSE}
...
startDate <- ymd_hms("2022-03-01 00:00:00")
endDate <- ymd_hms("2023-3-01 23:00:00")

# Sensor water quality carpentry
wq <- read.csv("data/wq/wq.csv", header= T) %>%
  filter(Site != 0) 

wq$Obs_Date <- as.Date(wq$Obs_Date, "%Y-%m-%d")
...
#Remove 2017
wq <- wq %>%
  subset(year %in% c('2018','2019','2020','2021','2022','2023')) %>%
...
ggplot() +
  ...
  scale_x_datetime(breaks = seq(startDate,
                 endDate, "4 weeks"), date_labels = "%b %d %Y",expand = c(0, 0)) +
...
**Figure 1-1.**  Salinity and river discharge data collected from autonomous sensors from the Lone Cabbage Reef restoration site near Suwannee, FL. Each graph represents a sensor location, with top of page as north, and right of page as east. The right column of figures (Sites 1-3) represent the eastern side of the Lone Cabbage Reef restoration site. The left column of figures (Sites 4-6) represent the western side of the Lone Cabbage Reef restoration site. The primary y-axis is Salinity (ppt, parts per thousand), and the secondary y-axis is Suwannee River discharge (CFS, cubic feet per second) measured at USGS Wilcox station 02323500 on the Suwannee River. River discharge is graphed as a daily mean in the light blue filled shape near the bottom of each graph. Daily mean salinity values (black line) are depicted using a 95% confidence interval (grey shaded region). Missing river discharge or salinity values are due to corrupt readings or missing equipment.

```{r alltemp_plots_processing, include= FALSE, warning=FALSE}


wq_temp <- wq %>%
  subset(year %in% c('2018','2019', '2020', '2021', '2022')) %>%
  subset(Site %in% c(1:9))
...
wq_temp2<- rbind(wqt1,wqt2,wqt3,wqt4,wqt5,wqt6)

wq_temp2$Site <- factor(wq_temp2$Site, levels = c("6", "1", "5", "2", "4", "3"))
...
ggplot() +
  ...
  scale_x_datetime(breaks = seq(startDate,
                 endDate, "2 weeks"), date_labels = "%b %d %Y",expand = c(0, 0)) +
...
**Figure 1-4.**  Temperature and discharge data collected from autonomous sensors from the Lone Cabbage Reef restoration site near Suwannee, FL. Each graph represents a sensor location, with top of page as north, and right of page as east. The figures (Sites 4-6) represent the western side of the Lone Cabbage Reef restoration site. The primary y-axis is Temperature (C, Celsius), and the secondary y-axis is Suwannee River discharge (CFS, cubic feet per second) measured at USGS Wilcox station 02323500 on the Suwannee River. River discharge is graphed as a daily mean in the light blue filled shape near the bottom of each graph. Daily mean temperature values (orange line) are depicted with a 95% confidence interval (shaded orange region). Missing river discharge or salinity values are due to corrupt readings or missing equipment.
